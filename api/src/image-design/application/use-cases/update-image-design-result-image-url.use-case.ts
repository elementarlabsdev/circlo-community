...
import { UpdateImageDesignResultImageUrlDto } from '../dtos/image-design.dto';

@Injectable()
export class UpdateImageDesignResultImageUrlUseCase {
  constructor(
    @Inject(IMAGE_DESIGN_REPOSITORY)
    private readonly coverRepository: IImageDesignRepository,
  ) {}

  async execute(id: string, userId: string, dto: UpdateImageDesignResultImageUrlDto): Promise<ImageDesignEntity> {
    ...
    design.update({ resultImageUrl: dto.resultImageUrl });
