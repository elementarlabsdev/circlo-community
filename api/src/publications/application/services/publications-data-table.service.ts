...
// Minimal DataTable config for publications.
// Note: DataTableService operates on scalar fields of the entity.
// Relations (author/channel/status) are not joined here but can be rendered on the client if needed.
...
@Injectable()
export class PublicationsDataTableService extends DataTableService {
  constructor(prisma: PrismaService) {
    super(prisma, config);
  }

  // Enforce only published publications in admin table
  async query(dto: DataTableQueryDto): Promise<DataTableResponseDto> {
    const {
      page = 1,
      pageSize = 10,
      sortBy,
    ...
    if (dynamicWhere && dynamicWhere.AND && Array.isArray(dynamicWhere.AND)) {
      ...
      // Fallback just in case buildWhere shape changes
