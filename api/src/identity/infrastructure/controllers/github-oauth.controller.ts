...
import { GithubOauthGuard } from '@/identity/infrastructure/guards/github-oauth.guard';
...
export class GithubOauthController {
  ...
  async callback(@Req() req: any, @Res({ passthrough: true }) res: Response) {
    ...
    if (req.user.emails?.length > 0) {
      const primaryEmail = req.user.emails.find(
    ...
    if (!oauthProvider) {
      ...
      await this._prisma.userOAuthLoginProvider.update({
        ...
        data: {
          profileUrl: oauthProvider.profileUrl || req.user.profileUrl,
          username: oauthProvider.username || req.user.username,
          data: req.user._json,
        },
      });
    }
