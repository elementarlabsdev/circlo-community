...
@Component({
  selector: 'app-digitalocean-file-storage-provider',
  imports: [
    ReactiveFormsModule,
    Button,
    DialogActions,
  ...
  templateUrl: './digitalocean-file-storage-provider.dialog.html',
  styleUrl: './digitalocean-file-storage-provider.dialog.scss'
})
export class DigitaloceanFileStorageProviderDialog {
  ...
  form = this._formBuilder.group({
    ...
    cdnEnabled: [this._data?.cdnEnabled ?? false],
  });
  ...
  apply() {
    ...
    this._api.post('admin/settings/file-storage/provider/digitalocean', {
      isEnabled: payload.isEnabled,
      cdnEnabled: payload.cdnEnabled,
      ...
      next: (res: any) => {
        this._snackBar.open(this._translateService.instant('admin.settings.fileStorage.digitalocean.saved'), '', {
          duration: 2500,
      ...
      error: () => {
        this._snackBar.open(this._translateService.instant('admin.settings.fileStorage.digitalocean.failed'), '', {
          duration: 3000,
          verticalPosition: 'top'
        });
      }
