...
export class ListComponent implements OnInit {
  ...
  async downloadSelected(): Promise<void> {
    ...
    try {
      const url = this.api.getApiEndpoint() + 'admin/media/download';
  ...
  private loadList() {
    this.api.get('admin/media/list').subscribe((res: any) => {
  ...
  async deleteSelected(): Promise<void> {
    ...
    try {
      const url = this.api.getApiEndpoint() + 'admin/media/delete';
      ...
      if (view === 'list') {
        ...
        if (Array.isArray(current) && current.length) {
          ...
          await new Promise<void>((resolve) => {
            this.api.get('admin/media/list').subscribe((r: any) => {
  ...
  onTableFilesChecked(event: any) {
    this.selectedFiles = (event?.selectedRowModel?.flatRows || []).map((row: any) => row.original);
  }
  ...
  private uploadOne(vm: UploadFileViewModel, file: File): void {
    const url = this.api.getApiEndpoint() + 'admin/media/upload';
  ...
  saveMediaView() {
    this.selectedFiles = [];
    this.api
      .post('admin/media/view', this.settingsForm.value)
      .subscribe((res: any) => {});
  }
}
